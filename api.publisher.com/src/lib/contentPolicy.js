...
export function analyzeCommunityContent(body, { platforms = [] } = {}) {
  ...
  if (containsForbiddenDash(text)) {
    issues.push({
      severity: 'error',
      message: 'Em dashes (—) are not allowed in posts. Use a comma or period instead.',
    })
  }
...
export function buildTextForPlatform(postState, platform) {
  ...
  const tags = (postState?.hashtags || [])
    .filter((h) => h.platforms?.[platform] && h.tag)
    .map((h) => sanitizePublishedText(h.tag.startsWith('#') ? h.tag : `#${h.tag}`))
  return sanitizePublishedText(tags.length ? `${body}\n\n${tags.join(' ')}`.trim() : body)
}
