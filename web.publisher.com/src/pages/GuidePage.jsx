...
const GUIDES = [
  ...
  },
  {
    ...
    redirect: 'https://publisher-suite.vercel.app/api/auth/linkedin/callback',
    steps: [
  ...
  },
  {
    ...
    redirect: 'https://publisher-suite.vercel.app/api/auth/reddit/callback',
    steps: [
  ...
  },
  {
    ...
    portal: ['Google Cloud Console', 'https://console.cloud.google.com/apis/credentials'],
    fields: ['OAuth Client ID', 'Client Secret'],
    redirect: 'https://publisher-suite.vercel.app/api/auth/gmail/callback',
    steps: [
    ...
    <section className="surface-panel rounded-xl p-5">
      <div className="flex items-center gap-3">
        {g.key === 'meta' ? (
          ...
          <span className="flex h-10 w-10 items-center justify-center rounded-[28%] bg-[#EA4335] text-sm font-bold text-white">M</span>
        ) : (
        ...
        <div className="min-w-0">
          ...
          <a href={g.portal[1]} target="_blank" rel="noreferrer" className="text-xs text-violet-300 hover:text-violet-200">
            {g.portal[0]} ↗
          </a>
        </div>
      </div>
    ...
    <PageShell>
      ...
      <PageScroll className="space-y-4 pb-4">
        <div className="rounded-xl border border-violet-500/20 bg-violet-500/[0.06] p-4 text-sm text-slate-300">
          ...
          <a href="/api-config" className="font-semibold text-violet-300 hover:text-violet-200">API Config</a>. Tokens are
          stored encrypted against your workspace and never shared across tenants. Follow the steps below to obtain each value.
        </div>
