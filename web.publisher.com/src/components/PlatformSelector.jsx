          ...
          >
            <div
              className={`flex h-10 w-10 items-center justify-center rounded-lg text-white ${
                p.gradient ? 'instagram-gradient' : ''
              }`}
              style={!p.gradient ? { backgroundColor: p.color } : undefined}
            >
              {p.icon}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate font-semibold text-sm text-white">{p.name}</p>
              <p className="truncate text-xs text-slate-500">{p.suite}</p>
            </div>
            <Toggle
              enabled={enabled}
